(function (global) {
  ...
  // Inline replacement for the Lucide icon library (previously loaded from
  // unpkg.com, ~80KB compressed just for icons). Path data extracted
  // directly from lucide@0.469.0 so every icon renders pixel-identical to
  // before. Covers every icon used in the markup plus a broad set an admin
  // could plausibly type into the popup's "fact icon" field — anything else
  // falls back to "sparkles" instead of rendering nothing.
  var ICON_PATHS = {
    ...
    'list-music': '<path d="M21 15V6"></path><path d="M18.5 18a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Z"></path><path d="M12 12H3"></path><path d="M16 6H3"></path><path d="M12 18H3"></path>',
    ...
    'mic-2': '<path d="m11 7.601-5.994 8.19a1 1 0 0 0 .1 1.298l.817.818a1 1 0 0 0 1.314.087L15.09 12"></path><path d="M16.5 21.174C15.5 20.5 14.372 20 13 20c-2.058 0-3.928 2.356-6 2-2.072-.356-2.775-3.369-1.5-4.5"></path><circle cx="16" cy="7" r="5"></circle>',
  ...
  function buildSvgMarkup(name, extraClass) {
    var inner = ICON_PATHS[name] || ICON_PATHS.sparkles;
    var cls = 'lucide lucide-' + name + (extraClass ? ' ' + extraClass : '');
    return SVG_OPEN + ' class="' + cls + '">' + inner + '</svg>';
  }
  ...
  function createIcons(root) {
    (root || document).querySelectorAll('[data-lucide]').forEach(function (el) {
      ...
      span.innerHTML = buildSvgMarkup(name, extraClass);
      var svg = span.firstChild;
      if (el.parentNode) el.parentNode.replaceChild(svg, el);
    });
  }

  global.AOIcons = { createIcons: createIcons, buildSvgMarkup: buildSvgMarkup };
